package com.act.quzhibo.luban_compress;
...
public class Luban {
    ...
    private static String DEFAULT_DISK_CACHE_DIR = "image";
    ...
    public Luban launch() {
        if(mFile==null){
            Log.e("mFile", "the image file cannot be null, please call .load() before this method!");
        }
    ...
    public Luban load(ArrayList<File> filePath) {
        mFileList = filePath;
        return this;
    }
    ...
    private File thirdCompress(@NonNull File file) {
        ...
        if (scale <= 1 && scale > 0.5625) {
            if (height < 1664) {
                ...
                size = size < 100 ? 100 : size;
            } else if (height >= 4990 && height < 7990) {
                thumbW = width / 3;
                thumbH = height / 3;
                size = (thumbW * thumbH) / Math.pow(2560, 2) * 300;
                size = size < 150 ? 150 : size;
            } else if (height >= 7990 && height < 10240) {
                thumbW = width / 4;
                thumbH = height / 4;
                size = (thumbW * thumbH) / Math.pow(2560, 2) * 300;
    ...
    private Bitmap compress(String imagePath, int width, int height) {
        ...
        int wid = outW > outH ? outH : outW;
        int hei = outW > outH ? outW : outH;

        int heightRatio = (int) Math.ceil(hei / (float) height);
        int widthRatio = (int) Math.ceil(wid / (float) width);
